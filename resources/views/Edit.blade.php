            ...
            <div class="row">
                <div class="col-lg-6 mx-auto">
                    <div class="card mt-5">
                        ...
                        <div class="card-body">
                            <form action="{{ route('update',$infos->id) }}" method="POST">
                                @csrf
                                <label for="">Student Name</label>
                                <input type="text" name="name" id=""
                                    class="form-control mb-3 @error('name') is-invalid @enderror"
                                    placeholder="Student Name" value="{{ $infos->name }}">
                                @error('name')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                                <label for="">Choose a Department:</label>
                                <select name="option" id=""
                                    class="form-control @error('option') is-invalid @enderror" value="{{ $infos->option }}">
                                ...
                                <input type="text" name="topic" id=""
                                    ...
                                    placeholder="Homework Topic" value="{{ $infos->topic }}">
                                @error('topic')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-lg-3 my-auto">
                                    <button class="btn btn-primary"><a href="{{route('edit',$info->id)}}" class="text-white text-decoration-none">Edit</a></button>
                                    <button class="btn btn-primary"><a href="{{route('delete',$info->id)}}" class="text-white text-decoration-none">Delete</a></button>
                                    <span class="btn btn-sm mt-3 btn-{{$info->status == 0 ? 'danger' : 'success'}}">{{$info->status == 0 ? 'Incomplete' : 'Complete'}}</span>
